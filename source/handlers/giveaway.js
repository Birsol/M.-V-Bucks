class Giveaway {
    ...
    async init() {
        this.msg = await this.channel.buildEmbed()
            .setColor(0xcc8822)
            .setTitle(this.title)
            .setDescription(`Nombre de gagnants: ${this.winnerCount}\nRéagissez avec :tada: pour participer !`)
            .setFooter(`Finit à ` + new Date(this.endTime))
            .send();
    ...
    async intervalFunction() {
        ...
        if(Date.now() >= this.endTime){
            ...
            if (!list.length) {
                embed.description = `Gagnant: Pas de gagnant`;
                embed.footer.text = `Giveaway Fini`;
            ...
            if (this.winners.length) this.msg.channel.send(`Bravo, ${this.winners.map(u => u.toString()).join(", ")} ! Tu as gagné **${this.title}** !`);
        }
